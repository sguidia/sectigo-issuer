...
// SectigoClusterIssuer is the Schema for the sectigoclusterissuers API.
type SectigoClusterIssuer struct {
...
func (vi *SectigoClusterIssuer) GetIssuerTypeIdentifier() string {
	// ACTION REQUIRED: Change this to a unique string that identifies your cluster issuer
	return "sectigoclusterissuers.sectigo.opensource.io"
}

// issuer-lib requires that we implement the Issuer interface
// so that it can interact with our Issuer resource.
var _ v1alpha1.Issuer = &SectigoClusterIssuer{}

// +kubebuilder:object:root=true

// SectigoClusterIssuerList contains a list of SectigoClusterIssuer.
type SectigoClusterIssuerList struct {
	metav1.TypeMeta `json:",inline"`
	metav1.ListMeta `json:"metadata,omitempty"`
	Items           []SectigoClusterIssuer `json:"items"`
}

func init() {
	SchemeBuilder.Register(&SectigoClusterIssuer{}, &SectigoClusterIssuerList{})
}
