...
```console
kubectl apply -f https://github.com/guidise/sectigo-issuer/releases/download/v0.1.0/install.yaml
```

## Demo

You can run the sectigo-issuer on a local cluster with this command:
...
and to follow the steps below, replacing references to `sectigo-issuer` with the name of your project.
...
git clone https://github.com/guidise/sectigo-issuer.git .
...
Use the `SectigoIssuer` and `SectigoClusterIssuer` definitions as a starting point.
...
```
$ kubectl describe clusterissuers.sectigo.opensource.io clusterissuer-sample
...
    Type:                  Ready
Events:
  Type     Reason            Age                From                    Message
  ----     ------            ----               ----                    -------
  Normal   IssuerReconciler  13s                sectigo-issuer  First seen
  Warning  IssuerReconciler  13s (x3 over 13s)  sectigo-issuer  Temporary error. Retrying: failed to get Secret containing Issuer credentials, secret name: sectigo-issuer-system/clusterissuer-sample-credentials, reason: Secret "clusterissuer-sample-credentials" not found
  Normal   IssuerReconciler  13s (x3 over 13s)  sectigo-issuer  Success
```
And this:
...
Events:
  Type     Reason                        Age   From                    Message
  ----     ------                        ----  ----                    -------
  Normal   CertificateRequestReconciler  23m   sectigo-issuer  Initialising Ready condition
  Warning  CertificateRequestReconciler  23m   sectigo-issuer  Temporary error. Retrying: error getting issuer: Issuer.sectigo.opensource.io "issuer-sample" not found
  Normal   CertificateRequestReconciler  23m   sectigo-issuer  Signed
...
See [PR 10: Generate Kubernetes Events](https://github.com/guidise/sectigo-issuer/pull/10) for an example of how you might generate events in your issuer.
...
kubectl wait --for=condition=Ready --timeout=5s sectigoissuers.sectigo.opensource.io sectigoissuer-sample
kubectl wait --for=condition=Ready --timeout=5s  certificates.cert-manager.io certificate-by-sectigoissuer
```
...
# Build and install sectigo-issuer and run the E2E tests.
...
* A Docker image will be generated and published to `ghcr.io/cert-manager/sectigo-issuer/controller` with the chosen tag.
...
[GitHub New Release Page]: https://github.com/guidise/sectigo-issuer/releases/new
